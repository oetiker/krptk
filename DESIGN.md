...
- **Durability** ("permanent" is a durability claim, not just a policy one):
  ...
  and in-process scheduled backup to S3-compatible object storage (the SQLite
  file also stays Litestream-compatible for operators who prefer a sidecar).
...
- **Volume must be block-backed RWO** — EBS/PD, Ceph RBD, Longhorn,
  local-path. **Never NFS or CephFS**: SQLite's locking is unsafe on most
  network filesystems, and this is the single most likely way to corrupt a
  krptk install. The chart should refuse (or loudly warn about) an RWX
  storage class.
- **`updateStrategy` must not run two pods at once.** With RWO the new pod
  ...
  in replica count.
- **Maintenance can't be a `CronJob`.** A separate pod running
  `krptk backup`/`scrub` would need the same RWO volume, which the server
  pod holds. Hence backup and scrub are **built into the server** on an
  internal schedule, writing to S3-compatible object storage configured in
  the values file. (A CronJob that *calls the admin API* is fine — it just
...
uses*. You keep reading and writing `localStorage` / IndexedDB exactly as
now; krptk mirrors it, encrypted, to your server, and pulls other devices'
changes back in. The pitch is roughly "iCloud-style sync for whatever your
PWA already keeps in the browser", self-hosted.
...
**The one honest limitation**: IndexedDB has no native change notification,
so writes must go *through* the adapter to be seen. For an existing app whose
writes are scattered, the SDK also offers a **scan mode** — periodically diff
the store against a hash index and sync what moved. Slower and more CPU, but
it makes adoption a genuine drop-in while the app migrates its writes to the
wrapper at leisure.
...
- **Status model** exposed for UI: `synced | pending | offline | conflict |
  quota-exceeded | not-linked`. Ships with a tiny sync-status indicator and
...
- **v1**: change feed + SSE, batching and chunking, service-worker sync with
  ...
  **management UI** (dashboard, accounts, invites, app registry, settings,
  audit log), in-process backup to S3 + scrub.
- **v2 (only if needed)**: scan-mode adapter for legacy IndexedDB writes,
  sharing between users (wrap a record key to another identity's public key),
...
8. **Scan mode**: needed for retrofitting your existing apps, or will they all
   route writes through the adapter (letting scan mode wait for v2)?
